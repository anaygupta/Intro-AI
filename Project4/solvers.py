...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        #update model paramters w by subtracting it by the gradient of the loss, weighted by the learning rate a
        #param_vars is a list of parameter variables

        learning_rate = self.learning_rate
        total = len(param_vars)

        #iterate through all parameter variables
        for i in range(total):
            #return list of tuples in format (param_var, new_param_tensor)
            param_var = param_vars[i]
            grad_tensor = grad_tensors[i]

            new_param_tensor = param_vars[i] - (learning_rate * grad_tensor)
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        #new weight update = momentum * previous weight update - alpha*(the gradient of the loss)
        #new weight = current weight + new weight update
        ...
        #iterate through all parameter variables
        for i in range(total):
            #return list of tuples in format (var, new_tensor)
            param_var = param_vars[i]
            vel_var = vel_vars[i]
            grad_tensor = grad_tensors[i]

            new_vel_tensor = (momentum * vel_var) - (learning_rate * grad_tensor)
            #append new weight update values
            updates.append([vel_var, new_vel_tensor])

            #add weight update to current weight
            new_param_tensor = param_var + new_vel_tensor
            #append new WEIGHT values too?
            updates.append([param_var, new_param_tensor])
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        #in every new iteration, the parameters should be updates using the gradient of the loss of all the training data
        ...
        for iter_ in range(self.iterations):
            ...
            # returns a tuple of lists, where
                # the first list contains the training loss of each iteration and the second list contains the validation loss of each iteration.
            p1 = model.input_ph
            p2 = input_train_data
            p3 = target_ph
            p4 = target_train_data
            p5 = input_val_data
            p6 = target_val_data

            # final_tensor = [loss_tensor] + update_ops
            train_loss = session.run(loss_tensor, feed_dict = {p1 : p2, p3 : p4})
            temp = session.run(update_ops, feed_dict={p1 : p2, p3 : p4})
            val_loss = session.run(loss_tensor, feed_dict = {p1 : p5, p3 : p6})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        # for every iteration, parameters should be updated using the gradient loss of only one data point of the training data
        ...
        for iter_ in xrange(self.iterations):
            ...
            trainBatch = trainGenerator.next()
            validationBatch = valGenerator.next()

            p1 = model.input_ph
            p2 = trainBatch[0]
            p3 = target_ph
            p4 = trainBatch[1]
            p5 = validationBatch[0]
            p6 = validationBatch[1]
